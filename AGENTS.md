...
- For every UI-affecting change (including navigation, filters, pagination, theme, loading states, dialogs, and reusable controls), preserve the existing interaction pattern across related modules and verify desktop/mobile plus applicable light/dark, normal/loading/empty/dialog states. Core navigation and actions must remain reachable without clipping or overflow.
