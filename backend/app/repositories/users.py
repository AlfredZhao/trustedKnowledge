...
async def ensure_user_schema_for_connection(connection: oracledb.AsyncConnection) -> None:
    ...
    await _execute_ddl(
        ...
        create table t_english (
            english_id number generated by default as identity primary key,
            sequence_no number,
            category varchar2(50),
            base_expression varchar2(50),
            professional_sentence varchar2(255),
            chinese_translation varchar2(255),
            ...
            created_at timestamp default systimestamp not null,
            updated_at timestamp default systimestamp not null,
            constraint t_english_is_flagged_ck check (is_flagged in (0, 1))
        )
        """,
    )
...
async def _make_column_nullable_if_needed(cursor: Any, table_name: str, column_name: str) -> None:
    """Relax a legacy NOT NULL constraint without reapplying Oracle DDL to nullable columns."""
    escaped_table = table_name.upper()
    escaped_column = column_name.upper()
    await cursor.execute(
        """
        select nullable
        ...
        """,
        {"table_name": escaped_table, "column_name": escaped_column},
    )
    row = await cursor.fetchone()
    if row and row[0] == "N":
        await _execute_ddl(cursor, f"alter table {escaped_table} modify ({escaped_column} null)")


async def _sync_identity_start_with_table_maximum(cursor: Any, table_name: str, column_name: str) -> None:
    """Advance an existing identity generator after rows with explicit IDs were migrated."""
    escaped_table = table_name.upper()
    escaped_column = column_name.upper()
    await cursor.execute(
        """
    ...
    if await cursor.fetchone() is not None:
        await _execute_ddl(
            cursor,
            f"alter table {escaped_table} modify {escaped_column} "
