...
- 任务内容区提供 `内容辅助`：可基于当前编辑框内容提取约 50 字且不超过 100 字的摘要，上传或粘贴封面图片并以 Markdown 图片格式写入任务内容草稿，以及生成可复制的结构化封面生图提示词。生图提示词页默认先展示生成结果；主题/构图选择与风格、元素、材质、灯光、镜头、质量、限制项多选配置通过 `配置参数` 按需展开，配置先进入草稿，点击 `应用配置` 后刷新提示词、保存到浏览器 UI 状态并自动收起。模板支持 `{{title}}`、`{{summary}}`、`{{topic}}`、`{{subject}}`、`{{objects}}`、`{{composition}}`、`{{style}}`、`{{lighting}}`、`{{material}}`、`{{camera}}`、`{{quality}}`、`{{negativePrompt}}` 占位符，并可恢复默认。
