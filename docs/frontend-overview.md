...
- 任务内容区提供 `内容辅助`：可基于当前编辑框内容提取约 50 字且不超过 100 字的摘要，上传或粘贴封面图片并以 Markdown 图片格式写入任务内容草稿，以及生成可复制的 C4D 封面图提示词，提示词模板支持 `{{title}}`、`{{summary}}`、`{{topic}}` 占位符，可在界面中编辑、保存到浏览器 UI 状态或恢复默认。
