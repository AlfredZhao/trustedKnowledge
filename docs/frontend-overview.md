...
| `users` | 用户管理 | admin 专用；管理 `TK_USERS` 用户、密码重置、状态/角色和 `TK_RELATIONS` 家长关系。 |
...
- 仅 `admin` 登录后显示在导航中。
- 普通用户不会看到入口；如果直接访问该视图，前端会回到总览，后端接口也会返回 403。

功能:

- 查看 `TK_USERS` 用户列表、角色、状态、是否已有密码、家长/孩子关系数量。
- 新建普通用户、家长用户或管理用户，并设置初始密码。
- 修改显示名、角色和启停状态。
