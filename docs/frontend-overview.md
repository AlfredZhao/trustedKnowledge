...
- 复制 Markdown 文章。
- Markdown 文章新增 `增强美化` 复制按钮：保留原始 Markdown 复制不变，并额外输出适合公众号粘贴的 HTML 富文本样式；段落标题默认使用暗红系强调样式，便于直接粘贴到公众号编辑器后保留层级感。
- `增强美化` 复制会额外尝试抓取正文中的可访问图片地址，并把图片以内联 base64 的方式写入剪贴板 HTML；若图片地址在浏览器中不可访问，则保留原地址，不阻断复制。
- 博客工厂中的 `增强美化` 复制还会同时下载一个完整 `.html` 文件到本地；文章导出默认按文章标题命名，任务内容导出默认按任务/知识编号命名，方便直接用浏览器打开核对图片和排版。
- Markdown 文章区新增 `载入任务内容` 按钮，可将当前任务内容一键填入正文编辑框。
