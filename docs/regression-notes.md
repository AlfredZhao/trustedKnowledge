...
`useMarkdownEditorPreview` 必须作为可信知识和待办共用的状态流程，通过 `MarkdownImageTextarea` 的 imperative handle 保存并恢复 `selectionStart`、`selectionEnd`、`scrollTop` 和 `scrollLeft`。切换记录时必须清除上一条记录的光标和滚动快照；同一时刻只能让一个可交互编辑器绑定该快照。恢复前还必须将 textarea 滚入外层页面或移动端 Sheet 视口。切到预览时，以保存的光标位置提取当前 Markdown 行；`MarkdownPreview` 必须为段落内的每一行及每个列表项保留独立源码行锚点，不能只定位连续图片所在段落的第一行。对未完成加载的图片，在短暂过渡期内应于 `load/error` 后重新定位，但滚轮、触摸、指针或键盘操作后必须停止自动滚动。预览中的滚动不得改写编辑快照；按钮和快捷键必须调用同一切换函数。
...
补全须创建按当前用户和请求指纹去重的后台任务。创建成功后，前端必须保持 `isCompleting` 为真，直到轮询收到 `completed`、`failed` 或 `cancelled` 终态；不得因 POST 已返回而重新显示“生成补全建议”。轮询短暂失败时必须保留 job ID 和 sessionStorage 记录，显示重试反馈后继续轮询；仅明确的“任务不存在”（例如后端重启）才可终止并清理。服务端须在 100 秒总时限内终止任务并返回明确失败状态；同标签页刷新后可按相同完整口播稿恢复轮询。完成结果仍仅供用户确认回填，不能自动保存或改写完整口播稿。英语素材补全的 Skill 在前端和请求模型中均最多一个。

## 守护测试

模拟一个未完成的补全任务，确认创建任务后加载状态不会闪退、生成按钮不会再次可点，重复创建会返回同一任务；让首次轮询失败后确认任务 ID 仍保留并自动重试；取消后状态为 `cancelled` 并显示“AI 补全已取消”。验证两个 skill ID 会被请求模型拒绝。手工验证 CLI 默认模型下的正常完成、超时、取消和刷新后重开弹窗恢复轮询，确认桌面/移动端与深浅主题中取消和状态提示均可达；运行 `cd frontend && npm run build`。

# 英语学习卡的 Skill 区块必须稳定保存且可独立复制

## 症状

通过带 `learning-card.json` 的 Skill 补全英语素材后，默认区块没有回填、保存后丢失，或修改该 Skill 后历史学习卡内容被悄悄改变。

## 触发条件

在英语素材 `AI补全` 中选中一个学习卡模板 Skill；确认回填并保存素材，随后重新打开详情、下载学习卡或修改该 Skill 的 JSON 配置。

## 根因

将 Skill 的模板定义直接当作历史素材展示数据，或仍沿用位置型 `ext1…ext5` 字段，导致配置没有受控传递、旧记录无法复现或字段含义漂移。

## 安全模式

后端只从当前 Agent 已授权的 Skill 固定读取 `learning-card.json`，校验后将 `default` 转为 `T_ENGLISH.CARD_SECTIONS` 的原生 JSON 快照；一次补全最多允许一个带模板文件的 Skill。详情和下载仅使用保存的快照，`visible` 控制显示，`copyable` 控制非空区块的独立复制按钮。

## 守护测试

验证 `补全学习卡@Alfred` 返回 `素材类型：影视双语字幕` 快照；选择两个模板 Skill 必须返回清晰冲突提示。保存后重新读取详情和下载 HTML，确认该区块仍存在并可单独复制；修改 Skill 默认值后，既有素材保持原值，新补全才使用新值。验证桌面/移动端、深浅主题和空值/隐藏区块，并运行 `python -m pytest backend/tests/test_english_materials.py` 与 `cd frontend && npm run build`。
